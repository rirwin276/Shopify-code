...
const roomScript=fs.readFileSync('assets/ss-anonymous-demo-start.js','utf8');
const roomMarkup=fs.readFileSync('snippets/ss-demo-wait-content.liquid','utf8').replace(/{%[\s\S]*?%}/g,'').replace(/{{[\s\S]*?}}/g,'');
...
function page() {
  ...
  <section data-ss-demo-start data-enabled="true" data-embedded="true" data-api-base="https://preview.example" hidden>${roomMarkup}</section>`;
}
...
test('the live-looking guest form starts a private build and saves its browser session', async () => {
  ...
  const w = dom.window; let request, acceptRequest; let submissions=0;
  ...
  w.fetch=async(url,options)=>{if(url.endsWith('/status'))return {ok:true,json:async()=>({phase:'building',build_stage:'products'})};submissions++;request={url,options};return new Promise(resolve=>{acceptRequest=()=>resolve({ok:true,json:async()=>({resume_token:'private-token',storefront_handle:'raptors-demo-a1b2c3'})});});};
  let destination=''; w.__ssAnonymousNavigate=(url)=>{destination=url};
  w.eval(roomScript); w.eval(script);
  ...
  const submitting=w.submitAnonymousPreview(new w.Event('submit'));
  assert.equal(w.document.querySelector('[data-ss-demo-start]').hidden,false);
  ...
  acceptRequest(); await submitting;
  ...
  assert.equal(destination,''); // Keep this very same waiting room on screen.
  ...
  assert.equal(w.document.getElementById('sf-provision').classList.contains('sf-hidden'),true);
  assert.equal(w.document.querySelector('[data-storefront-request-form]').hidden,true);
  assert.equal(w.document.querySelector('[data-ss-demo-start]').hidden,false);
  dom.window.close();
});
